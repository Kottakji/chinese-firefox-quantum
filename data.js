...
ppcDict.prototype = {
    ...
    loadDictionary: function () {
        if (ppcMain.config.dialect === 'cantonese') {
            this.wordDict = this.fileRead(chrome.extension.getURL("data/cantonese/dict.dat"));
            this.wordIndex = null;
    ...
    wordSearch: function (word) {
        ...
        // The cantonese search
        let data = [];
        let matchLen = 0;
        while (word.length > 0) {

            let start = this.wordDict.regexIndexOf("\n" + word + "\t");
            if (start !== -1) {
                start++;  // Add one, as we started looking at the \n before
                let end = this.wordDict.regexIndexOf("\n[\u4E00-\u9FCC]", start + 1); // This fails on the very last line?
                let line = this.wordDict.substring(start, end);

                line = line.replace(/\\n[0-9]\./, ""); // First occurence of \n1. should be replaced with a ""
                line = line.replace(/\\n[0-9]\./g, "<br>");
                line = line.replace(/\\n/g, ""); // Left over new lines should be blank

                line = line.replace(/([\u4E00-\u9FCC]+).+?\[(.+?)](.+)/g, "$1 $1 [$2] /$3/");
                data.push(line);

                // The length starts at the first " " space.
        ...
        return {data: [data], matchLen: matchLen};
    },
